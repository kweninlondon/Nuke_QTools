...
independently choose **ZIP1** or **DWAA** EXR compression, choose **all**, **rgba**,
or **rgb** channels, toggle **Autocrop** (enabled by default), and enable **Match
frame range**. These output choices are applied to the Write node inside the
created AYON group. Select multiple rows to edit any dropdown or checkbox across
the selection. Batch buttons set the type or range matching for all rows. Range
matching copies the nearest upstream Read's first and last frames to the created
group's internal Write, enables its range limit, and updates the group's exposed
range knobs when they are available.
